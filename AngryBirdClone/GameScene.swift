...
//  AngryBirdClone
//
//  Created by Mehmet Ekşi on 21.02.2023.
//
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if gameStarted == false{
            if let touch = touches.first{
                let touchLocatotion = touch.location(in: self)
                let touchNodes = nodes(at: touchLocatotion)
                
                if touchNodes.isEmpty == false{
                    for node in touchNodes {
                        if let sprite = node as? SKSpriteNode{
                            if sprite == bird{
                                bird.position = touchLocatotion
                            }
                        }
                    }
                }
            ...
            if let touch = touches.first{
                ...
                if touchNodes.isEmpty == false{
                    for node in touchNodes {
                        if let sprite = node as? SKSpriteNode{
                            if sprite == bird{
                                let dx = -(touchLocatotion.x - originalPosition!.x)
                                let dy = -(touchLocatotion.y - originalPosition!.y)
